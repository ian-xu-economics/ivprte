...
#'
#' @param target.parameter The target parameter to compute the average weights for.
...
#' @param late.lb Required when `target.parameter` is set to "LATE", specifies the lower bound for integration.
#' @param late.ub Required when `target.parameter` is set to "LATE", specifies the upper bound for integration.
...
#'
#' @export
compute_average_weights = function(target.parameter = c("AUO", "ATO", "ATE", "ATT", "ATU", "LATE"),
                                   dgp = NULL,
                                   data = NULL,
                                   late.lb = NULL,
                                   late.ub = NULL) {
  if(target.parameter == "AUO"){
    weights <- data.frame(uStart = 0,
                          uEnd = 1,
                          avgWeightD1 = 0,
                          avgWeightD0 = 1)
  } else if(target.parameter == "ATO"){
    weights <- data.frame(uStart = 0,
                          uEnd = 1,
                          avgWeightD1 = 1,
                          avgWeightD0 = 0)
    ...
    weights <- matrix(data = NA_real_,
                      nrow = length(uPoints)-1,
                      ncol = 4,
                      dimnames = list(NULL,
                                      c("uStart",
                                        "uEnd",
    ...
    if(late.lb > 0){
      weightLower <- data.frame(uStart = 0,
                                uEnd = late.lb,
                                avgWeightD1 = 0,
                                avgWeightD0 = 0)
    } else{
    ...
    if(late.ub < 1){
      weightUpper <- data.frame(uStart = late.ub,
                                uEnd = 1,
                                avgWeightD1 = 0,
                                avgWeightD0 = 0)
    } else{
